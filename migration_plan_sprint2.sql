...
BEGIN
    -- 创建交互菜单配置表
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'interaction_menu_configs') THEN
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'character_follows') 
       AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_subscriptions') THEN
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'subscription_groups') THEN
...
BEGIN
    -- 交互日志索引
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'interaction_logs') THEN
    ...
    -- 记忆条目索引
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'memory_items') THEN
