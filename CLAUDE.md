...
[GitHub PR自动提交配置]
    为了解决Claude Code中自动提交PR的认证问题，我们已配置了以下解决方案：
    ...
    **该脚本会自动：**
    - 检查GitHub CLI认证状态
    - 如未认证则自动使用存储的token进行认证
    - 认证完成后可直接使用 gh pr create 等命令
    
    **使用说明：**
    1. 确保脚本有执行权限：chmod +x gh-auto-auth.sh
    2. 每次新的Claude Code会话开始时运行一次该脚本
    3. 认证完成后即可正常使用GitHub CLI进行PR操作
    
    **注意事项：**
    - token已安全存储在脚本中，请勿泄露
    - 该配置仅适用于joshua23账户的GitHub仓库
    - 如需更新token，请修改脚本中的GH_TOKEN变量